...
import { DepartmentService } from '../../ng-sevice/department.service'
...
export class FilterService {
    ...
    getdepartmentList(data): Observable<any> {
        ...
        return this.https.$post(this.powerService.getViewApiUrl({ urlKey: 'deptListByPower' }), 'CRMAPISERVER', {}).pipe(
            switchMap(res => {
    ...
    getCustomerEnum(type): Observable<any> {
        ...
        return this.https.$get('/sys/enums', 'CRMAPISERVER',{
            clsName: type
        }).pipe(
            switchMap(res => {
                if(!res.success){ this.message.error(res.message); return of([]) }
